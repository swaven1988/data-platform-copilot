...
def test_policy_warn_does_not_block_build(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        PolicyEngine(policies=[always_warn]),
    )

    job_name = "policy_warn_job"
    ...
    assert any(p.get("code") == "TEST_POLICY_WARN" for p in policy), br
    assert not any(p.get("status") in ("FAIL", "BLOCK") for p in policy), br

    # Build should proceed (message varies by path)
    assert br.get("message") in (
        "Build V2 completed",
        "No changes detected. Skipping rebuild.",
        "Policy check failed",  # shouldn't happen for WARN, but keep safe if code changes
    )